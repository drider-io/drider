function onHashChange2(e) {
  console.log(e);
  var match = window.location.hash.match(/route_map(.+)$/)
  if (match){
    var id = match[1];
    var canvas = $('#route_map'+id);
    if (!canvas.is(':visible')){
      ...
      map.data.loadGeoJson('/car_routes/'+id+'.json')
    }

  }
}
$(window).on('hashchange', onHashChange2);
$( document ).ready(onHashChange2);
