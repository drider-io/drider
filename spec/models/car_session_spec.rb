...
RSpec.describe CarSession, type: :model do
  describe '::for_user' do
    ...
    context 'when session with expired location' do
      let!(:car_location) { create(:car_location, user: user, car_session: car_session, created_at: Time.now - 16.minutes ) }
    ...
    context 'when session with actual location' do
      before { create(:car_location, user: user, car_session: car_session ) }
